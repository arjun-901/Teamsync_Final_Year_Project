...
export const getWorkspaceMembersService = async (
  ...
  if (role === Roles.MEMBER) {
    ...
    const sharedProjectMembers = await ProjectMemberModel.find({
      workspace: workspaceId,
      project: {
        $in: memberProjects.map((item) => item.project),
      },
    }).select("userId");
    ...
    visibleUserIds = Array.from(
      new Set([
        userId,
        ...sharedProjectMembers.map((item) => item.userId.toString()),
      ])
    );
  }
...
export const getWorkspaceAnalyticsService = async (
  ...
  let projectFilter: mongoose.Types.ObjectId[] | null = null;
  ...
  if (role === Roles.MEMBER) {
    ...
    projectFilter = memberships.map((item) => item.project);
  }
  ...
  const [
    ...
    ProjectModel.countDocuments({
      workspace: workspaceId,
      ...(projectFilter ? { _id: { $in: projectFilter } } : {}),
    }),
  ]);
